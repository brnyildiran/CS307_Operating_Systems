...
public class Philosopher extends Thread {
    ...
    public Philosopher(int i, Semaphore [] s, int [] my_state, int philosopher_number, Semaphore mu, Semaphore[] bar)
    {
        id = i;
        sem = s;
        state = my_state;
        N = philosopher_number;
        mutex = mu;
        barriers = bar;
    }
    ...
    public static void main(String args[]) {
        ...
        for (int i=0; i < N; i++)
        {
            semarray[i] = new Semaphore(1,true);//changed the semaphore value from 0 --> 1
    ...
    //CS 307 HW2 -- Starts Here ----------------------------------------------------------------------------------------

    //----- Barrier Implementation -----
    public void Barrier_for_Philosophers()
    {
        try
        {
            for(int j = 0; j < 4; j++)
            ...
            for(int j = 0; j < 5; j++)
            {
                if(id != j) //Down others except this semaphore
                {
                    barriers[j].acquire();
                }
            }
    ...
    //----- EATING State of Philosophers ----- take_fork/ eat/ put_fork
    public void eat()
    {
        int are_you_eating = 0;
        while(are_you_eating != 1) //While philosopher has NOT eating
        {
            if(sem[id].tryAcquire()) //If Left Fork is AVAILABLE --> Make a down on Left Fork's semaphore
            {
                if(sem[(id+4)%5].tryAcquire()) //If Right Fork is AVAILABLE --> Make a down on Right Fork's semaphore
                {
                    state[id]= EATING;
                    System.out.println("Philosopher " + id + " is eating. ");
                    table.ForkTake_GUI(id);//Right & Left Fork --> red
                    table.Eating_GUI(id);//Spaghetti --> blue
                    table.ForkPut_GUI(id);//Right & Left Fork --> white
                    table.StopEating_GUI(id);//Spaghetti --> white
                    sem[id].release();//release Left Fork
                    sem[(id+4)%5].release();//release Right Fork
                    are_you_eating = 1;
                }
                else//Left fork available BUT right fork is NOT AVAILABLE
                {
                    sem[id].release();//release Left Fork
                }
            }
   ...
   //----- THINKING State of the Philosophers ----- think/ get_hungry
    public void think()  //Make philosopher think for --> random # of seconds
    {
        //Thinks
        state[id]= THINKING;

        Random think_time = new Random();
        int  rand_think_time  = think_time.nextInt(10000);

        System.out.println("Philosopher " + id + " is thinking for " + rand_think_time/1000.0 + " seconds.");
        try {
            sleep(rand_think_time);
        }
        catch (InterruptedException e)
        {
            e.printStackTrace();
        ...
        System.out.println("Philosopher " + id + " is hungry. ");//After thinking --> Philosopher hungry
        table.Hungry_GUI(id);//Spaghetti --> red
        state[id]= HUNGRY;//Philosopher state --> hungry
    }
    ...
    public void run()
    {
        System.out.println("Philosopher " + id + " has entered the room.");
        Random arrive_time = new Random();
        int  rand_arrive_time = arrive_time.nextInt(10000);
        System.out.println("Philosopher " + id + " walking to the table in " + rand_arrive_time/1000.0 + " seconds.");
        try {
            sleep(rand_arrive_time);
        }
        catch (InterruptedException e)
        {
            e.printStackTrace();
        }
        table.PutPlate_GUI(id);//Give philosopher a plate when s/he arrives -->yellow spaghetti
        Barrier_for_Philosophers();//Set up the barrier for waiting the other philosophers
        table.StartDining_GUI(id);//All philosophers arrived --> start dining
        while(true)
        {
            think();
            eat();
